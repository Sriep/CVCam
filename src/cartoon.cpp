...
// Detects the color of the pixels in the middle of the image, then changes the color of that region to green.
void Cartoon::changeFacialSkinColor(Mat smallImgBGR, Mat bigEdges, int debugType)
{
...
void Cartoon::removePepperNoise(Mat &mask)
{
...
void Cartoon::drawFaceStickFigure(Mat dst)
{
    Size size = dst.size();
    int sw = size.width;
    int sh = size.height;
