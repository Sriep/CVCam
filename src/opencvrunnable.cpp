...
QVideoFrame SketchVFRunnable::runWithImageWrapper(QVideoFrame *input, const QVideoSurfaceFormat &surfaceFormat, QVideoFilterRunnable::RunFlags flags)
{
...
void OpenCVRunnable::changeFacialSkinColor(cv::Mat smallImgBGR, cv::Mat bigEdges, int debugType)
{
        ...
        drawFaceStickFigure(smallImgBGR);
...
void OpenCVRunnable::removePepperNoise(cv::Mat &mask)
{
...
void OpenCVRunnable::drawFaceStickFigure(cv::Mat dst)
{
    Size size = dst.size();
    int sw = size.width;
    int sh = size.height;
...
void OpenCVRunnable::cartoonifyImage(Mat inMat, Mat dst, bool sketchMode, bool alienMode, bool evilMode, int debugType)
{
    ...
    Mat srcGray;
    cvtColor(inMat, srcGray, CV_BGR2GRAY);
    ...
    resize(inMat, smallImg, smallSize, 0,0, INTER_LINEAR);
